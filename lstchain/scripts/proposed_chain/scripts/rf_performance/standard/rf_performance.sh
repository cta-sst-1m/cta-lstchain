...
if [[ "${cluster}" == "camk" ]]; then
  ...
  echo "Not implemented yet"
  echo sbatch --job-name=rf_perform_${intensity}_${leakage} --output=rf_perform_.out --error=rf_perform_.err --export=json_config_file=${json_config_file},dl1_gamma_test=${dl1_gamma_test},dl1_gamma_train=${dl1_gamma_train},dl1_proton_test=${dl1_proton_test},dl1_proton_train=${dl1_proton_train},input_folder=${input_folder},cam_key=${cam_key},output_folder=${output_folder},working_dir=${working_dir} ${working_dir}/${cluster}.sbatch
